...
2. **$git status:** 현재 작업 중인 디렉터리와 스테이징 영역의 상태를 확인
...
4. **$git commit 파일 -m "로그메세지":** 변경 사항을 Git 저장소에 영구적으로 기록, 
Staging Area에서 Repository로 이동하는 것

5. **reset**: 원하는 시점으로 돌아간 뒤 이후 내역들 삭제.
...
6. **$git revert [commit id]:** 되돌리기 원하는 시점의 커밋을 거꾸로 실행.
